...
<div x-data="{
    ...
    >
        <input type="file" {{ $multiple ? 'multiple' : '' }} @if($accept) accept="{{ $accept }}" @endif @change="handleSelect($event)" {{ $attributes->except(['size', 'multiple', 'accept', 'description']) }} aria-label="Upload files">
