...
Route::group(['middleware' => 'guest'], function () {
    ...
    Route::post("register", [RegisterController::class, 'registerPost'])->middleware('throttle:5,1')->name("register.post");
    Route::get('resend-verification-email', [RegisterController::class, 'resendVerificationEmail'])->name('resend.verification.email');
    Route::post('resend-verification-email', [RegisterController::class, 'resendVerificationEmailPost'])->middleware('throttle:3,1')->name('resend.verification.email.post');
    Route::get('verify-email/{token}', [RegisterController::class, 'verifyEmail'])->name('verify.email');

    Route::get("forgot-password", [ForgotPasswordController::class, 'forgotPassword'])->name("forgot.password");
    Route::post("forgot-password", [ForgotPasswordController::class, 'sendResetLinkEmail'])->middleware('throttle:3,1')->name("forgot.password.post");
    Route::get("reset-password/{token}", [ForgotPasswordController::class, 'showResetForm'])->name("reset.password");
    Route::post("reset-password", [ForgotPasswordController::class, 'resetPassword'])->middleware('throttle:5,1')->name("reset.password.post");
});
